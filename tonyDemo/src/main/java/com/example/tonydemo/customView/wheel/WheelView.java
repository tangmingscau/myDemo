...
public class WheelView extends View {
    private static String TAG = WheelView.class.getSimpleName();
    /**
     * 控件宽度
     */
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                Log.i(WheelView.class.getSimpleName(), "ACTION_DOWN");
                break;
            ...
            case MotionEvent.ACTION_UP:
                ...
                // 判断段时间移动的距离
                LogUtils.i(WheelView.class.getSimpleName(), "ACTION_UP");
                upY = (int) event.getY();
                if (Math.abs(y - downY) < 3) {
                    clickActionUp(y);
                } else {
                    if (System.currentTimeMillis() - downTime < goonTime
                            && move > goonDistence) {
                        LogUtils.i(WheelView.class.getSimpleName(), "短促移动");
                        goonMove(y - downY);
                    } else {
                        LogUtils.i(WheelView.class.getSimpleName(), "actionUp");
                        actionUp(y - downY);
                    }
                }
                noEmpty();
    ...
    private void clickActionUp(int y) {

        int newMove = 0, newY = 0, selectY = 0;
        for (int i = 0; i < itemList.size(); i++) {
            if (i <= itemList.size() - 2) {
                if (y - (itemList.get(i).y - unitHeight / 4) > 0 && y - (itemList.get(i + 1).y - unitHeight / 4) < 0) {
                    newY = itemList.get(i).y;
                }
            } else {
                if (y - (itemList.get(i).y - unitHeight / 4) > 0)
                    newY = itemList.get(i).y;
            }
            if (itemList.get(i).isSelected()) {
                selectY = itemList.get(i).y;
            }
        }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        offset = attribute.getDimension(R.styleable.WheelView_offset, 20f);
        ...
        public void drawSelf(Canvas canvas, int containerWidth) {
            ...
            if (isSelected()) {
                ...
                moveToSelect = moveToSelect > 0 ? moveToSelect : moveToSelect
                        * (-1);
                marginleft = (int) (offset * (1.0f - (float) moveToSelect
                        / (float) unitHeight));
                // 计算当前字体大小
            ...
            if (!isInView())
                return;
            // 绘制内容
            canvas.drawText(itemText, x + marginleft, y + move + unitHeight / 2 + textRect.height() / 2,
                    textPaint);
